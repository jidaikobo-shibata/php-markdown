...
### v1保守TODO: tableのコロン判定

- v2で修正した次の2点を、将来のv1保守リリースでも検討する。
  - 旧caption行は、表本文の最終行かつ第2セル以降が空の場合だけcaptionとして扱う。
  - 行見出しの末尾コロンは、実際に最後となるインライン内容にある場合だけ扱う。
- v1はMichelfベースの文字列処理であり、v2のAST実装をそのままcherry-pickしない。
  `1.x`から作業ブランチを作り、`src/Traits/TableTrait.php`とv1用回帰テストへ限定して
  同じ判定規則を移植する。
- 実装前に、v1の`parseSpan()`でインラインコード等が保護されるため、コロン判定の
  境界問題がすでに再現しない可能性をテストで確認する。再現しない部分は挙動を変えず、
  回帰テストだけを追加する。
- 最低限のテスト対象は、途中行の先頭コロン、最終行だが他セルが空でないケース、
  正常な旧caption行、コロンより後ろにインラインコードがあるセルとする。
- 未完了: v1へのコード変更、v1環境での回帰テスト、リリース番号の決定。
- 次にやるとよいこと: v1保守が必要になった時点で`1.x`から作業ブランチを作り、
  再現テストを先に追加してから必要最小限の修正を行う。

### 再利用可能なチートシート断片と安定版前監査

- 英日Markdown原稿へ非表示の再利用開始マーカーを追加し、原稿の導入部と、
  `目次`／`Contents`から末尾までの記法ヘルプを生成時に分離するようにした。
- CSSや`main`、ページタイトルを含まない英日HTML断片を`resources/cheatsheet/`へ生成し、
  `CheatSheet::getHtml()`から言語allowlistを通して取得できるようにした。
- CMS側でサンプル資材のbase URLと見出しID接頭辞を指定できるようにした。URLは
  絶対HTTP(S)または単一スラッシュのルート相対URLだけを受け付け、接頭辞は安全な
  ASCII文字に限定する。既定の相対URLで直接利用できる小さな資材も断片と同梱した。
- Pico CSS版とBootstrap版は、同じMarkdown原稿から生成するリポジトリ確認用ページ
  として維持した。利用側へCSSフレームワークを要求しない方針をREADMEへ記載した。
- 安定版前監査で見つかった`mixed`型境界を整理し、CommonMarkノードからHTML属性を
  取り出す際は文字列・真偽値・文字列リストだけを通すようにした。`parse_url()`の
  戻り値と設定マージの型も明確化した。
- 理由: CMSの既存ランドマークとCSSを尊重しつつ記法ヘルプを再利用可能にし、
  ID衝突・パストラバーサル・属性注入・配布資材欠落を安定版前に防ぐため。
- 回帰テストでは断片の開始・範囲、CSS非包含、言語allowlist、資材URL、ID接頭辞、
  ID重複と全フラグメントリンクの移動先、複製資材の同期を確認した。
- Composer auditは既知の脆弱性0件。PHPStanは設定済みlevel 5と追加のlevel maxで0件。
  PSR-12、PHP 7.4互換検査、Composer strict validate、生成物同期も成功した。
- 現在の作業ツリーを複製したComposer配布候補ZIPにHTML断片と資材が含まれること、
  ZIP展開先から`CheatSheet::getHtml()`が動くことを確認した。動的5ページ、静的4ページ、
  サンプル資材2点はPHP組み込みサーバーですべてHTTP 200となり、サーバーは終了した。
- 既知の許容事項: PHP 7.4は静的互換検査のみ、CIは未導入、CommonMark 2.10.0を維持する。
- 未完了: 利用者によるCMS組み込み時の表示確認、変更のコミット、v2.0.0向け文書・タグ・
  pushなどのリリース作業。
- 次にやるとよいこと: HTML断片を実案件相当のCMSコンテナーへ組み込み、見出し階層、
  CSS、キーボード操作を目視確認してからv2.0.0リリース作業へ進む。

### v2.0.0リリース準備

- 再利用可能なチートシート断片と監査時の型境界改善をコミット`5f6e9c9`として確定した。
- READMEの導入コマンドを安定版系列の`^2.0`へ変更し、現在版を`2.0.0`とした。
- 移行ガイドからbeta固有の案内を外し、主要バージョン更新前に代表コンテンツと連携を
  検証する案内へ変更した。
- CHANGELOGではbeta.2後の変更を2026-09-09付けの`2.0.0`として確定し、将来の変更用に
  空のUnreleased節を残した。
- 理由: beta.1・beta.2での表示確認、セキュリティ修正、API拡張、配布物監査を経て、
  Composerが通常のstable制約で導入できるv2系列を公開するため。
